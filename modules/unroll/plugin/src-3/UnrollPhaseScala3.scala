...
class UnrollPhaseScala3() extends PluginPhase {
  ...
  private def isValidUnrolledMethod(method: Symbol, origin: SrcPos)(using Context) = {
    ...
    def explanation =
      ...
      else if isSyntheticCaseCompanionMethod(method) then
    ...
    if method.name.is(DefaultGetterName) then false
      ...
      || isSyntheticCaseCompanionMethod(method)
      || isSyntheticCaseCopy(method) then
  ...
  // The compiler-generated `apply` / `fromProduct` of a case class companion, and `copy` of a case
  // class, get their parameters (and `@unroll` annotations) from the primary constructor. Only these
  // are special-cased below: hand-written overloads of the same names are ordinary methods, that are
  // left alone (unless they carry `@unroll` annotations themselves).
  private def isSyntheticCaseCompanionMethod(method: Symbol)(using Context) =
    method.is(Synthetic) &&
    (method.name == nme.apply || method.name == nme.fromProduct) &&
    method.owner.companionClass.is(CaseClass)

  private def isSyntheticCaseCopy(method: Symbol)(using Context) =
    method.is(Synthetic) && method.name == nme.copy && method.owner.is(CaseClass)

  // A hand-written overload with the same signature as a forwarder about to be generated (like an
  // `apply` overload in a case class companion). It takes precedence over the forwarder, as on
  // Scala 2 where nothing gets generated for it.
  private def existingOverload(original: Symbol, forwarder: Symbol)(using Context): Option[Symbol] =
    original.owner.info.decls.lookupAll(original.name).find { s =>
      s != original && s.is(Method) && s.info.matches(forwarder.info)
    }
  ...
  def generateSyntheticDefs(tree: Tree)(using Context): (Option[Symbol], Seq[Tree]) = tree match{
    case defdef: DefDef if defdef.paramss.nonEmpty =>
      ...
      val isCaseCopy = isSyntheticCaseCopy(defdef.symbol)

      val isCaseApply = isSyntheticCaseCompanionMethod(defdef.symbol) && defdef.name == nme.apply

      val isCaseFromProduct = isSyntheticCaseCompanionMethod(defdef.symbol) && defdef.name == nme.fromProduct
      ...
      // `dataclassCopy` drops the type parameter clause of the `copy` of a `@data` case class,
      // while the parameters below still come from the primary constructor, that keeps one: shift
      // the parameter clause indices of `copy` accordingly.
      val paramClauseOffset =
        if (isCaseCopy) defdef.paramss.length - annotated.paramSymss.length
        else 0
                ...
                (-1 +: annotationIndices :+ paramCount).sliding(2).toList.foldLeft((Seq.empty[DefDef], defdef.symbol)) {
                  case ((defdefs, nextSymbol), Seq(paramIndex, nextParamIndex)) =>
                    val forwarder = generateSingleForwarder(
                      ...
                      paramIndex,
                      nextSymbol,
                      paramClauseIndex + paramClauseOffset,
                      defdef.paramss,
                      isCaseApply
                    )
              ...
              (
                ...
                (annotationIndices :+ paramCount).sliding(2).toList.reverse.foldLeft((Seq.empty[DefDef], defdef.symbol)){
                  case ((defdefs, nextSymbol), Seq(paramIndex, nextParamIndex)) =>
                    ...
                    // Skip the forwarder if an overload with the same signature is defined by hand,
                    // and have the shorter forwarders call that overload instead.
                    existingOverload(defdef.symbol, forwarder.symbol) match {
                      case Some(existing) => (defdefs, existing)
                      case None           => (forwarder +: defdefs, forwarder.symbol)
                    }
                }._1
              )
            }
