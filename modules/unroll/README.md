...
Setters (`withX`):
- The plugin generates none, while the data-class macro generates one per field on Scala 2. Rather
  than hand-writing them for Scala 3, sources shared by both call `copy` instead
  (`app.copy(launcherType = …)` rather than `app.withLauncherType(…)`), which both compilers
  provide. Only setters that do more than set a field (wrapping a value in `Some`, say) are written
  by hand, in the shared sources, where they already were on Scala 2.
