...
class TestState {
  ...
  val sparkParentJson = {
    val content = inMemoryCache.fromCache("https://repo1.maven.org/maven2/org/apache/spark/spark-parent_2.12/2.4.0/spark-parent_2.12-2.4.0.pom")
    val p = MavenRepository.parseRawPom(content).right.get
    val s = JsonProj.write(p)
    System.err.println(s"JSON size: ${s.length}")
    s
  }
  ...
  System.err.println("State initialized")
