...
private[coursier] class MavenRepositoryInternal(
  ...
  // `MavenRepository.create` strips a trailing '/' from the root, but the case class `apply`
  // (and user-provided roots) may keep one; normalize here so URL building never yields a
  // double slash regardless of how the repository was constructed.
  private def rootUrl: String = root.stripSuffix("/")

  def urlFor(path: Seq[String], isDir: Boolean = false): String = {
    val b = new StringBuilder(rootUrl)
    b += '/'
