...
class CliFetchIntegrationTest extends FlatSpec with CliTestLib with Matchers {
  ...
  "Normal fetch" should "get all files" in {
    ...
    val (_, files) = Fetch
      .task(params, pool, Seq("junit:junit:4.12"))
      .unsafeRun()(ec)
  ...
  "scalafmt-cli fetch" should "discover all main classes" in {
    ...
    val params = paramsOrThrow(options)
    val (_, files) = Fetch
      .task(params, pool, Seq("com.geirsson:scalafmt-cli_2.12:1.4.0"))
      .unsafeRun()(ec)
    val loader = new URLClassLoader(files.map(_._2.toURI.toURL).toArray, Launch.baseLoader)
    assert(
      Launch.mainClasses(loader) == Map(
  ...
  "scalafix-cli fetch" should "discover all main classes" in {
    ...
    val params = paramsOrThrow(options)
    val (_, files) = Fetch
      .task(params, pool, Seq("ch.epfl.scala:scalafix-cli_2.12.4:0.5.10"))
      .unsafeRun()(ec)
    val loader = new URLClassLoader(files.map(_._2.toURI.toURL).toArray, Launch.baseLoader)
    assert(
      Launch.mainClasses(loader) == Map(
  ...
  "ammonite fetch" should "discover all main classes" in {
    ...
    val params = paramsOrThrow(options)
    val (_, files) = Fetch
      .task(params, pool, Seq("com.lihaoyi:ammonite_2.12.4:1.1.0"))
      .unsafeRun()(ec)
    val loader = new URLClassLoader(files.map(_._2.toURI.toURL).toArray, Launch.baseLoader)
    assert(
      Launch.mainClasses(loader) == Map(
  ...
  "sssio fetch" should "discover all main classes" in {
    ...
    val params = paramsOrThrow(options)
    val (_, files) = Fetch
      .task(params, pool, Seq("lt.dvim.sssio:sssio_2.12:0.0.1"))
      .unsafeRun()(ec)
    val loader = new URLClassLoader(files.map(_._2.toURI.toURL).toArray, Launch.baseLoader)
    assert(
      Launch.mainClasses(loader) == Map(
  ...
  "Module level" should "exclude correctly" in withFile("junit:junit--org.hamcrest:hamcrest-core") { (file, _) =>
    withFile() { (jsonFile, _) =>
  ...
  /**
    ...
    */
  "avro exclude xz" should "not fetch xz" in withFile("org.apache.avro:avro--org.tukaani:xz") { (file, writer) =>
    withFile() { (jsonFile, _) =>
      val dependencyOpt = DependencyOptions(localExcludeFile = file.getAbsolutePath)
      val resolveOpt = ResolveOptions(dependencyOptions = dependencyOpt)
      val options = FetchOptions(jsonOutputFile = jsonFile.getPath, resolveOptions = resolveOpt)
      ...
      val (_, files) = Fetch
        .task(params, pool, Seq("org.apache.avro:avro:1.7.4"))
        .unsafeRun()(ec)
      ...
      assert(
        node.dependencies.map(_.coord).toSet == Set(
          "org.apache.avro:avro:1.7.4",
          "com.thoughtworks.paranamer:paranamer:2.3",
          "org.apache.commons:commons-compress:1.4.1",
          "org.codehaus.jackson:jackson-core-asl:1.8.8",
  ...
  /**
    ...
    * |   └─ org.tukaani:xz:1.0
    */
  "avro excluding xz + commons-compress" should "still fetch xz" in withFile("org.apache.avro:avro--org.tukaani:xz") {
    (file, writer) =>
      withFile() { (jsonFile, _) =>
        {
          val dependencyOpt = DependencyOptions(localExcludeFile = file.getAbsolutePath)
          ...
          val (_, files) = Fetch
            .task(params, pool, Seq("org.apache.avro:avro:1.7.4", "org.apache.commons:commons-compress:1.4.1"))
            .unsafeRun()(ec)
          val filesFetched = files.map(_._2.getName).toSet
          assert(filesFetched.contains("xz-1.0.jar"))
  ...
  /**
    ...
    * |└─ org.tukaani:xz:1.1
    */
  "requested xz:1.1" should "not have conflicts" in withFile() { (excludeFile, writer) =>
    withFile() { (jsonFile, _) =>
      {
        ...
        Fetch
          .task(params, pool, Seq("org.apache.commons:commons-compress:1.4.1", "org.tukaani:xz:1.1"))
          .unsafeRun()(ec)
      ...
      withFile() { (jsonFile, _) =>
        {
          ...
          Fetch
            .task(params, pool, Seq("org.apache.commons:commons-compress:1.5", "org.tukaani:xz:1.1"))
            .unsafeRun()(ec)
  ...
  /**
    ...
    * |   └─ org.tukaani:xz:1.2
    */
  "classifier tests" should "have tests.jar" in withFile() { (excludeFile, _) =>
    withFile() { (jsonFile, _) =>
      {
        val options = FetchOptions(jsonOutputFile = jsonFile.getPath)
        val params = paramsOrThrow(options)
  ...
  /**
    ...
    * |   └─ org.tukaani:xz:1.2
    */
  "mixed vanilla and classifier " should "have tests.jar and .jar" in withFile() { (excludeFile, _) =>
    withFile() { (jsonFile, _) =>
      {
        val options = FetchOptions(jsonOutputFile = jsonFile.getPath)
        val params = paramsOrThrow(options)
        ...
        val compressNodes: Seq[DepNode] = node
          .dependencies
        ...
        compressNodes
          .last.file.map(f => assert(f.contains("commons-compress-1.5-tests.jar"))).orElse(fail("Not Defined"))
      }
    }
  }
  ...
  /**
    ...
    */
  "intransitive" should "only fetch a single jar" in withFile() { (_, _) =>
    withFile() { (jsonFile, _) =>
      {
  ...
  /**
    ...
    * |   └─ org.tukaani:xz:1.2
    */
  "intransitive classifier" should "only fetch a single tests jar" in withFile() { (excludeFile, _) =>
    withFile() { (jsonFile, _) =>
      {
        val dependencyOpt =
          DependencyOptions(intransitive = List("org.apache.commons:commons-compress:1.5,classifier=tests"))
        val resolveOpt = ResolveOptions(dependencyOptions = dependencyOpt)
        val options = FetchOptions(jsonOutputFile = jsonFile.getPath, resolveOptions = resolveOpt)
        val params = paramsOrThrow(options)

        Fetch
          .task(params, pool, Seq())
          .unsafeRun()(ec)
  ...
  /**
    ...
    * |   └─ org.tukaani:xz:1.0
    */
  "classifier with forced version" should "fetch tests jar" in withFile() { (excludeFile, _) =>
    withFile() { (jsonFile, _) =>
      {
        ...
        assert(compressNode.isDefined)
        compressNode
          .get.file.map(f => assert(f.contains("commons-compress-1.4.1-tests.jar"))).orElse(fail("Not Defined"))
  ...
  /**
    ...
    */
  "intransitive, classifier, forced version" should "fetch a single tests jar" in withFile() { (excludeFile, _) =>
    withFile() { (jsonFile, _) =>
      {
  ...
  "profiles" should "be manually (de)activated" in withFile() { (jsonFile, _) =>
  ...
  "com.spotify:helios-testing:0.9.193" should "have dependencies with classifiers" in withFile() { (excludeFile, _) =>
    withFile() { (jsonFile, _) =>
      {
        val options = FetchOptions(jsonOutputFile = jsonFile.getPath)
        val params = paramsOrThrow(options)
        ...
        assert(testEntry.dependencies.exists(_.startsWith("com.spotify:docker-client:jar:shaded:")))
        assert(node.dependencies.exists(_.coord.startsWith("com.spotify:docker-client:jar:shaded:")))
      }
    }
  }
  ...
  /**
    ...
    * |└─ a:b:c
    */
  "local file dep url" should "have coursier-fetch-test.jar and cached for second run" in withFile() { (jsonFile, _) =>
    {
      withFile("tada", "coursier-fetch-test", ".jar") { (testFile, _) =>
        {
          ...
          val depNodes1: Seq[DepNode] = node1
            .dependencies
          ...
          val depNodes2: Seq[DepNode] = node2
            .dependencies
            .filter(_.coord == "a:b:c")
            .sortBy(fileNameLength)
  ...
  /**
    ...
    * |└─ org.apache.commons:commons-compress:1.5
    */
  "external dep url" should "fetch junit-4.12.jar" in withFile() { (jsonFile, _) =>
    {
      val options = FetchOptions(jsonOutputFile = jsonFile.getPath)
      val params = paramsOrThrow(options)
  ...
  /**
    ...
    * |└─ h:i:j
    */
  "external dep url with arbitrary coords" should "fetch junit-4.12.jar" in withFile() { (jsonFile, _) =>
    {
      val options = FetchOptions(jsonOutputFile = jsonFile.getPath)
      val params = paramsOrThrow(options)
  ...
  /**
    ...
    */
  "classifier sources" should "fetch sources jar" in withFile() { (jsonFile, _) =>
    {
      ...
      depNodes
        .head.dependencies.foreach(d => {
          assert(d.contains(":sources:"))
        })

      assert(coords == Seq("org.apache.commons:commons-compress:jar:sources:1.5", "org.tukaani:xz:jar:sources:1.2"))
    }
  }
    ...
    (jsonFile, _) =>
      {
        ...
        compressNodes
          .head.file.map(f => assert(f.contains("junit/junit/4.12/junit-4.12.jar"))).orElse(fail("Not Defined"))
        ...
        jacksonMapperNodes
          .head.file.map(
            f => assert(f.contains("org/codehaus/jackson/jackson-mapper-asl/1.8.8/jackson-mapper-asl-1.8.8.jar"))
          ).orElse(fail("Not Defined"))
        ...
        jacksonCoreNodes
          .head.file.map(
            f => assert(f.contains("org/codehaus/jackson/jackson-core-asl/1.8.8/jackson-core-asl-1.8.8.jar"))
          ).orElse(fail("Not Defined"))
      }
  }
  ...
  /**
    ...
    *  Error
    */
  "external dep url with forced version" should "throw an error" in withFile() { (jsonFile, _) =>
    {
      val resolutionOpt = ResolutionOptions(forceVersion = List("org.apache.commons:commons-compress:1.4.1"))
      val resolveOpt = ResolveOptions(
        resolutionOptions = resolutionOpt
  ...
  /**
    ...
    * |└─ org.apache.commons:commons-compress:1.5
    */
  "external dep url with the same forced version" should "fetch junit-4.12.jar" in withFile() { (jsonFile, _) =>
    {
  ...
  /**
    ...
    */
  "external dep url on higher version" should "fetch junit-4.12.jar" in withFile() { (jsonFile, _) =>
    {
      val options = FetchOptions(jsonOutputFile = jsonFile.getPath)
      val params = paramsOrThrow(options)
  ...
  /**
    ...
    * |   └─ org.tukaani:xz:1.2
    */
  "external dep url on lower version" should "fetch higher version" in withFile() { (jsonFile, _) =>
    {
      val options = FetchOptions(jsonOutputFile = jsonFile.getPath)
      val params = paramsOrThrow(options)
  ...
  "Bad pom resolve" should "succeed with retry" in withTempDir("tmp_dir") { dir =>
    {
      def runFetchJunit() = {
        ...
        assert(
          files
            .map(_._2.getName).toSet
            .equals(Set("junit-4.12.jar", "hamcrest-core-1.3.jar"))
        )
        val junitJarPath = files.map(_._2.getAbsolutePath()).filter(_.contains("junit-4.12.jar")).head
  ...
  "Bad jar resolve" should "succeed with retry" in withTempDir("tmp_dir") { dir =>
    {
      def runFetchJunit() = {
        val cacheOpt = CacheOptions(cache = dir.getAbsolutePath)
        val resolveOpt = ResolveOptions(cacheOptions = cacheOpt)
  ...
  "Wrong range partial artifact resolve" should "succeed with retry" in withTempDir("tmp_dir") { dir =>
    {
      def runFetchJunit() = {
        ...
        val params = paramsOrThrow(options)
        val (_, files) = Fetch
          .task(params, pool, Seq("junit:junit:4.6"))
          .unsafeRun()(ec)
        assert(
          files
            .map(_._2.getName).toSet
        ...
        val junitJarPath = files.map(_._2.getAbsolutePath()).filter(_.contains("junit-4.6.jar")).head
        Paths.get(junitJarPath)
      }
