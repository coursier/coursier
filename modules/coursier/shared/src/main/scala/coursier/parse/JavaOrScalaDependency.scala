...
object JavaOrScalaDependency {
  ...
  @data case class JavaDependency(dependency: Dependency, exclude: Set[JavaOrScalaModule])
    ...
    def dependency(
      ...
      dependency.copy(
        minimizedExclusions = dependency.minimizedExclusions.join(
          MinimizedExclusions(
  ...
  @data case class ScalaDependency(
    ...
    def dependency(
      ...
      baseDependency
        .copy(
          module = baseDependency.module.copy(name = ModuleName(newName)),
          minimizedExclusions =
    ...
    for (classifierOpt <- userParams.get(classifierKey).flatMap(_.headOption))
      classifierOpt match {
        case Some(classifier) =>
          ...
          csDep = csDep.copy(
            publication = csDep.publication.copy(classifier = Classifier(classifier))
          )
        case None =>
    ...
    for (extOpt <- userParams.get(extKey).flatMap(_.headOption))
      extOpt match {
        case Some(ext) =>
          ...
          csDep = csDep.copy(
            publication = csDep.publication.copy(ext = Extension(ext))
          )
        case None =>
          errors += "Invalid empty classifier attribute"
      }
    for (typeOpt <- userParams.get(typeKey).flatMap(_.headOption))
      typeOpt match {
        case Some(tpe) =>
          ...
          csDep = csDep.copy(
            publication = csDep.publication.copy(`type` = Type(tpe))
          )
        case None =>
          errors += "Invalid empty classifier attribute"
      }
