...
@data case class Fetch[F[_]](
  ...
  def withCache(cache: Cache[F]): Fetch[F] =
    copy(resolve = resolve.copy(cache = cache), artifacts = artifacts.copy(cache = cache))
  ...
  def addArtifactTypes(artifactTypes: Type*): Fetch[F] =
    copy(
      artifacts = artifacts.copy(
        artifactTypesOpt = Some(artifacts.artifactTypesOpt.getOrElse(Set()) ++ artifactTypes)
      )
    )
  ...
  def addVariantAttributes(attributes: (String, VariantSelector.VariantMatcher)*): Fetch[F] =
    copy(
      resolve = resolve.copy(
        resolutionParams = resolve.resolutionParams.addVariantAttributes(attributes: _*)
      )
    )
...
object Fetch {
  ...
  @data case class Result(
    ...
    def withFullDetailedArtifacts(artifacts: Seq[(
      ...
      copy(
        fullDetailedArtifacts0 = artifacts.map {
