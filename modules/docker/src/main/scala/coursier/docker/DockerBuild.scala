...
object DockerBuild {
  ...
  def layerArchive(
    ...
    if (entries.isEmpty)
      ...
      maybeSudo.sudoWith("tar", "--sort=name", "--mtime=@0", "-zcf", archivePath.toString, entries)(
        cwd = dir
      )
      ...
      inst.value match {
        ...
        case run: DockerInstruction.Run =>
          val res = DockerRun.run(
            ...
            withUpperDirArchive = Some { archive =>

              val archiveIsEmpty =
                Using.resource(os.read.inputStream(archive)) { is =>
                  new TarArchiveInputStream(is).getNextEntry() == null
                }
              val layerOpt =
                if (archiveIsEmpty) None
                else {
                  val dest = archive / os.up / s"${archive.last}.gz"
                  ...
                  Using.resources(os.read.inputStream(archive), os.write.outputStream(dest)) {
                    (is, os) =>
