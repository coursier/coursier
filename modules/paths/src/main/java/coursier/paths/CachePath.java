...
public class CachePath {
    ...
    public static Path localFile(String url, Path cache, String user, boolean localArtifactsShouldBeCached) throws MalformedURLException {
        return localFile(url, cache, user, localArtifactsShouldBeCached, FileSystems.getDefault());
    }
    ...
    public static Path localFile(
            ...
            Path cache,
            String user,
            boolean localArtifactsShouldBeCached,
            FileSystem fs) throws MalformedURLException {

        // use the getPath override accepting a URI in case of problem with the two cases below?

        if (url.startsWith("file:///") && !localArtifactsShouldBeCached)
            return fs.getPath(url.substring("file://".length()));
        ...
        return cache.resolve(escape(protocol + "/" + userPart + remaining));
    }
