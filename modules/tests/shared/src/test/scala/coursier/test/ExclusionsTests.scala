...
object ExclusionsTests extends TestSuite {
  ...
  def exclusionsAdd(e1: Set[(Organization, String)], e2: Set[(Organization, String)]): Set[(Organization, String)] =
    core.Exclusions.minimize(e1 ++ e2)

  val tests = Tests {
    val e1 = Set((org"org1", "name1"))
    val e2 = Set((org"org2", "name2"))

    val enb = Set((org"org1", "*"))
    val eob = Set((org"*", "name1"))
    val eb = Set((org"*", "*"))
