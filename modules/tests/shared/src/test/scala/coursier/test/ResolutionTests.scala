...
object ResolutionTests extends TestSuite {
  ...
  val projects = Seq(
    ...
    Project(mod"acme:play", "2.4.0", Seq(Configuration.empty -> dep"acme:play-json:2.4.0")),
    ...
    Project(
      ...
      "1.0.0",
      dependencyManagement = Seq(Configuration.`import` -> Dependency(Module(org"acme", name"missing-pom"), "1.0.0"))
    ),
    Project(
      Module(org"hudsucker", name"mail"),
      "10.0",
      Seq(Configuration.test -> Dependency(Module(org"$${project.groupId}", name"test-util"), "${project.version}"))
    ),
    ...
    Project(
      ...
      "18.0",
      dependencies = Seq(Configuration.empty -> Dependency(Module(org"acme", name"play"), "")),
      parent = Some(Module(org"se.ikea", name"parent"), "18.0")
    ),
    Project(
      Module(org"org.gnome", name"parent"),
      "7.0",
      Seq(Configuration.empty -> Dependency(Module(org"org.gnu", name"glib"), "13.4"))
    ),
    Project(
      Module(org"org.gnome", name"panel-legacy"),
      "7.0",
      dependencies = Seq(Configuration.empty -> Dependency(Module(org"org.gnome", name"desktop"), "${project.version}")),
      parent = Some(Module(org"org.gnome", name"parent"), "7.0")
    ),
    Project(
      Module(org"gov.nsa", name"secure-pgp"),
      "10.0",
      Seq(Configuration.empty -> Dependency(Module(org"gov.nsa", name"crypto"), "536.89"))
    ),
    Project(
    ...
    Project(
      ...
      "2.6",
      dependencies = Seq(Configuration.empty -> Dependency(Module(org"junit", name"junit"), "")),
      dependencyManagement = Seq(Configuration.test -> Dependency(Module(org"junit", name"junit"), "4.11"))
    ),
    Project(
    ...
    Project(
      ...
      profiles = Seq(
        Profile(
          ...
          dependencies =
            Seq(Configuration.empty -> Dependency(Module(org"org.escalier", name"librairie-standard"), "2.11.6"))
        )
      )
    ),
    Project(
      ...
      "0.4.2",
      dependencies =
        Seq(Configuration.empty -> Dependency(Module(org"org.scalaverification", name"scala-verification"), "1.12.4")),
      profiles = Seq(
        Profile(
          "default",
          activeByDefault = Some(true),
    ...
    Project(Module(org"com.github.dummy", name"libb-parent"), "0.5.6", properties = Seq("special" -> "true")),
    Project(
      Module(org"com.github.dummy", name"libb"),
      "0.5.6",
    ...
    Project(
      ...
      Seq(
        Configuration.empty -> Dependency(
          Module(org"an-org", name"a-lib"),
          "1.0",
          exclusions = Set((org"an-org", name"a-name"))
        ),
    ...
    Project(
      Module(org"an-org", name"an-app"),
      "1.1",
      Seq(Configuration.empty -> Dependency(Module(org"an-org", name"a-lib"), "1.1"))
    ),
    Project(
      Module(org"an-org", name"an-app"),
      "1.2",
      Seq(Configuration.empty -> Dependency(Module(org"an-org", name"a-lib"), "1.2"))
    ),
    ...
    Project(
      ...
      "1.0",
      Seq(Configuration.empty -> Dependency(Module(org"an-org", name"my-lib-1"), "1.0.0+build.027"))
    ),
    Project(
      Module(org"an-org", name"my-lib-3"),
      "1.0",
      Seq(Configuration.empty -> Dependency(Module(org"an-org", name"my-lib-1"), "1.1.0+build.018"))
    ),
    Project(
      Module(org"an-org", name"my-lib-3"),
      "1.1",
      Seq(Configuration.empty -> Dependency(Module(org"an-org", name"my-lib-1"), "1.2.0"))
    ),
    Project(
  ...
  val projectsMap =
    projects.map(p => p.moduleVersion -> p.copy(configurations = MavenRepository.defaultConfigurations)).toMap
  ...
  val tests = Tests {
    ...
    'exclude {
      async {
        ...
        val trDeps = Seq(
          Dependency(Module(org"acme", name"play"), "2.4.1", exclusions = Set((org"acme", name"config"))),
          Dependency(Module(org"acme", name"play-json"), "2.4.0", exclusions = Set((org"acme", name"config")))
        )
        val res = await(
          resolve0(
            Seq(dep)
    ...
    'excludeOrgWildcard {
      async {
        ...
        val trDeps = Seq(
          Dependency(Module(org"acme", name"play"), "2.4.1", exclusions = Set((org"*", name"config"))),
          Dependency(Module(org"acme", name"play-json"), "2.4.0", exclusions = Set((org"*", name"config")))
        )
        val res = await(
          resolve0(
            Seq(dep)
    ...
    'parentDepMgmt {
      async {
        ...
        val trDeps = Seq(
          Dependency(Module(org"acme", name"play"), "2.4.0", exclusions = Set((org"acme", name"play-json")))
        )
        val res = await(
          resolve0(
            Seq(dep)
    ...
    'propertiesInExclusions {
      async {
        ...
        val trDeps =
          Seq(Dependency(Module(org"gov.nsa", name"secure-pgp"), "10.0", exclusions = Set((org"*", name"crypto"))))
        val res = await(
          resolve0(
            Seq(dep)
          )
    ...
    'depsFromDefaultProfile {
      async {
        ...
        val trDeps = Seq(Dependency(Module(org"org.escalier", name"librairie-standard"), "2.11.6"))
        val res = await(
          resolve0(
            Seq(dep)
          )
    ...
    'dependencyOverrides - {
      * - {
        async {
          val deps = Seq(Dependency(Module(org"an-org", name"a-name"), "1.1"))
          val depOverrides = Map(Module(org"an-org", name"a-name") -> "1.0")
      ...
      * - async {
        val deps = Seq(Dependency(Module(org"an-org", name"an-app"), "1.1"))
        val depOverrides = Map(Module(org"an-org", name"a-lib") -> "1.0")
      ...
      * - async {
        val deps = Seq(Dependency(Module(org"an-org", name"an-app"), "1.1"))
        val depOverrides = Map(Module(org"*", name"a-lib") -> "1.0")
      ...
      * - {
        async {
          val deps = Seq(Dependency(Module(org"an-org", name"an-app"), "1.2"))
          val depOverrides = Map(Module(org"an-org", name"a-lib") -> "1.1")
