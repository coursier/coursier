...
object ArchiveCache {
  ...
  def apply(): ArchiveCache[Task] =
    create[Task]()
  def apply(location: File): ArchiveCache[Task] =
    create[Task](location)

  // Not `apply` overloads: the case class `apply` already has default arguments, and only one
  // overload may define some.
  def create[F[_]](
