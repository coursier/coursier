...
testBootstrap() {
  if [ "$SCALA_VERSION" = 2.12 ]; then
    ...
    ./coursier-test bootstrap -o cs-echo-launcher-args io.get-coursier:echo:1.0.0
    if [ "$(./cs-echo-launcher-args -Dother=thing foo -Dfoo=baz)" != foo ]; then
      echo "Error: unexpected output from bootstrapped echo command (generated by proguarded launcher)." 1>&2
      exit 1
    fi
    ...
    ./coursier-test bootstrap -o cs-props-args io.get-coursier:props:1.0.2
    if [ "$(./cs-props-args -Dfoo=baz)" != baz ]; then
      echo "Error: unexpected output from bootstrapped echo command (generated by proguarded launcher)." 1>&2
      exit 1
    fi
