...
class LauncherNative04(val cliScalaVersion: String) extends CsModule with CoursierPublishModule {
...
class CliTests(val cliScalaVersion: String) extends CsModule with CoursierPublishModule { self =>
  ...
  object test extends Tests with CsTests {
    def forkArgs = {
      val launcherTask = cli(ScalaVersions.scala212).launcher.map(_.path)
      val assemblyTask = cli(ScalaVersions.scala212).assembly.map(_.path)
      T {
  ...
  object `native-tests` extends NativeTests {
    def cliLauncher = cli(ScalaVersions.scala212).nativeImage
  }
  object `native-static-tests` extends NativeTests {
    def cliLauncher = cli(ScalaVersions.scala212).`static-image`.nativeImage
  }
  object `native-mostly-static-tests` extends NativeTests {
    def cliLauncher = cli(ScalaVersions.scala212).`mostly-static-image`.nativeImage
  }
  object `native-container-tests` extends NativeTests {
    def cliLauncher = cli(ScalaVersions.scala212).containerImage
  }
}
...
def simpleNative04CliTest() = T.command {
  `launcher-native_04`(ScalaVersions.scala212).publishLocal()()
  val launcher = cli(ScalaVersions.scala212).launcher().path
...
def copyLauncher(directory: String = "artifacts") = T.command {
  val nativeLauncher = cli(ScalaVersions.scala212).nativeImage().path
...
def copyStaticLauncher(directory: String = "artifacts") = T.command {
  val nativeLauncher = cli(ScalaVersions.scala212).`static-image`.nativeImage().path
...
def copyMostlyStaticLauncher(directory: String = "artifacts") = T.command {
  val nativeLauncher = cli(ScalaVersions.scala212).`mostly-static-image`.nativeImage().path
...
def copyContainerLauncher(directory: String = "artifacts") = T.command {
  val nativeLauncher = cli(ScalaVersions.scala212).containerImage().path
  ghreleaseassets.copyLauncher(
    nativeLauncher,
    os.Path(directory, os.pwd),
    suffix = "-container"
...
def uploadLaunchers(directory: String = "artifacts") = T.command {
  val version = cli(ScalaVersions.scala212).publishVersion()
...
def bootstrapLauncher(
  ...
  cli(ScalaVersions.scala212).run(Seq(
    "bootstrap",
    "-o",
    dest,
    "-f",
...
def nativeTests() = T.command {
  `cli-tests`(ScalaVersions.scala212).`native-tests`.test()()
}

def nativeStaticTests() = T.command {
  `cli-tests`(ScalaVersions.scala212).`native-static-tests`.test()()
}

def nativeMostlyStaticTests() = T.command {
  `cli-tests`(ScalaVersions.scala212).`native-mostly-static-tests`.test()()
}
