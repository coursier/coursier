...
trait UsesRedirectingServer extends Module {
  ...
  def redirectingServer = T.worker {
    ...
    server.proc = os.proc("java", "-Xmx128m", "-cp", cp.mkString(File.pathSeparator), mainClass, server.host, server.port.toString).spawn(
      stdin = os.Pipe,
      stdout = os.Inherit,
      stderr = os.Inherit
    )
...
def testRepoServer = T.worker {
  ...
  server.proc = os.proc(
    "cs", "launch", "io.get-coursier:http-server_2.12:1.0.0", "--",
    "-d", "modules/tests/handmade-metadata/data/http/abc.com",
    "-u", server.user,
    "-P", server.password,
    "-r", "realm",
    "-v",
