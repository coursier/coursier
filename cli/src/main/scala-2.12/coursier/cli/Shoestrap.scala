...
import caseapp._
import coursier.cli.options.ShoestrapOptions
import coursier.cli.util.Zip
import coursier.internal.FileUtil

import scala.collection.JavaConverters._

object Shoestrap extends CaseApp[ShoestrapOptions] {

  def run(options: ShoestrapOptions, args: RemainingArgs): Unit = {
    ...
    if (options.options.native) {
      ...
      val shoestrapJar =
        Option(Thread.currentThread().getContextClassLoader.getResourceAsStream("shoestrap.jar")) match {
          case Some(is) => Cache.readFullySync(is)
          case None =>
            Console.err.println(s"Error: shoestrap JAR not found")
            sys.exit(1)
        }
      ...
      val (urls, files) =
        helper.fetchMap(
          ...
          case ((urls, files), (url, file)) =>
            if (url.startsWith("file:/")) (urls, file :: files)
            else (url :: urls, files)
        }
      ...
      val bootstrapZip = new ZipInputStream(new ByteArrayInputStream(shoestrapJar))
